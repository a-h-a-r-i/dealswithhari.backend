...
async function resolveUrl(url) {
  try {
    // Use axios with no redirect following — capture Location header manually
    let current = url;
    for (let i = 0; i < 8; i++) {
      let res;
      try {
        res = await axios.get(current, {
          timeout: 8000,
          maxRedirects: 0,
          validateStatus: s => true, // don't throw on any status
          headers: {
      ...
      if ([301, 302, 303, 307, 308].includes(status) && location) {
        ...
        // If it's an app deep link (intent:// or fk-app://) — stop, use current web URL
        if (/^(intent|fk-app|flipkart-app):\/\//i.test(current)) break;
        continue;
      }
      // No more redirects
      break;
