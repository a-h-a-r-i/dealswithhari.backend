...
// ── Step 2: Build precise search query from title ────────────────────────────
function buildSearchQuery(rawTitle) {
  // Convert URL slug to words if needed
  ...
  return text
    .replace(/\(.*?\)/g, "")
    .replace(/\[.*?\]/g, "")
    // Only remove truly useless words — keep color, size, brand, model
    .replace(/\b(buy|online|india|free|shipping|latest|best|offer|deal|discount|review|price)\b/gi, "")
    .replace(/\s+/g, " ")
    .trim()
    // Keep up to 10 words for specificity
    .split(" ")
    .filter(Boolean)
    .slice(0, 10)
    .join(" ");
...
async function getComparisons(productUrl) {
  ...
  console.log(`🔍 Query: "${searchQuery}" (from title: "${rawTitle}")`);

  // Clear stale cache for this query to get fresh results
  cache.del(`gshopping:${searchQuery}`);
...
module.exports = { getComparisons, detectPlatform, buildAffiliateLink, scrapeTitle, resolveUrl };
