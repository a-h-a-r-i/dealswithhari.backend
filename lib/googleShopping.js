  ...
  return new Promise((resolve) => {
    ...
    const req = lib.get(url, {
      ...
      if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
        ...
        res.resume();
        resolveUrl(next).then(resolve);
      } else {
        res.resume();
        resolve(res.req?.path ? `${new URL(url).origin}${res.req.path}` : url);
      }
    });
...
async function getComparisons(productUrl) {
  // Resolve short links — detect by known shorteners OR very short paths
  const isShortLink = /dl\.flipkart\.com|amzn\.(in|to)|bit\.ly|tinyurl|goo\.gl/i.test(productUrl)
    || (new URL(productUrl).pathname.replace(/\//g, "").length < 15);
  const resolvedUrl = isShortLink ? await resolveUrl(productUrl) : productUrl;
