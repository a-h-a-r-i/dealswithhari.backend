...
async function scrapeTitle(url, platform) {
  ...
  try {
    ...
    try {
      const seg = new URL(url).pathname.split("/").filter(Boolean).find(
        s => s.length > 8 && !["dp","p","product","item"].includes(s.toLowerCase())
      );
...
async function searchGoogleShopping(query) {
  ...
  try {
    ...
    // Merge: sponsored first, then organic (dedupe by site)
    const seenSites = new Set();
    const results = [];
    for (const p of [...sponsoredResults, ...organicResults]) {
      if (!seenSites.has(p.site)) {
