...
async function resolveUrl(url) {
  try {
    ...
    return request.res?.responseUrl || request.responseURL || url;
  } catch (err) {
    // axios throws on non-2xx but we can still get the redirect location
...
async function getComparisons(productUrl) {
  // Resolve short links (dl.flipkart.com, amzn.in, etc.) to real URLs
