...
async function getPriceComparisons(productUrl) {
  ...
  console.log("BuyHatke raw response:", JSON.stringify(raw).slice(0, 300));

  // Parse response — handle various response shapes
  let responseArray: any[] = [];
  if (Array.isArray(raw)) responseArray = raw;
  else if (Array.isArray(raw?.data)) responseArray = raw.data;
  else if (raw?.data && typeof raw.data === "object") {
    // data is an object keyed by platform id
    responseArray = Object.values(raw.data).flat();
  }
